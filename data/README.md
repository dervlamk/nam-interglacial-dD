...
`tseries/{PI,LGM}/tseries_0801-0900/*_plev.nc` (HPC-side) — the pressure-level regrid of
`raw/tseries/` built by `scripts/ncl/pressureRegrid_tseries.ncl`.
...
`raw/tseries/` and `interim/tseries/` (HPC-side, above) are populated by running
...
is copied into the repo's own `data/raw/tseries/`. Everything else model-side still comes from
