# data/

**The directory tree is tracked; the data is not.** A fresh clone gets these four empty
directories so there is a defined place to download data to. Nothing in here except this
README and the small tracked tables in `processed/` is under version control.

`DATA_MANIFEST.md` at the repo root is the authoritative list of what each dataset is, where it
came from, and which files are still ambiguous. This file only says *where to put things*.

```
data/
├── raw/         Original, immutable. Never edited, never written to by any script.
├── external/    Third-party datasets not produced by this project.
├── interim/     Intermediate products of the pipeline.
└── processed/   Final, analysis-ready.
```

## What belongs where

### `raw/` — original and immutable

Laptop-side (proxy, downloaded/copied from OneDrive):

| | |
|---|---|
| `dD_nh22p.xls` | NH22P raw δD<sub>C30</sub> (sheet `dD_nh22p`) |
| `d480-479_dD.xlsx` | DSDP-480/479 raw δD<sub>C30</sub> (sheets `d480_dD_good`, `d479_dD_good`) |
| `nh22p_processed_dD_handpicked.xlsx` | NH22P record, hand-compiled from GC-IRMS output (manual peak-picking). **Primary data despite the "processed" name** — nothing generates it. |
| `d480_d479_processed_dD.xlsx` | as above, for DSDP-480/479 |
| `Bacon_runs/DSDP480/`, `Bacon_runs/DSDP479/` | Bacon age-depth model output. See the canonical-run note in `DATA_MANIFEST.md` before using DSDP-480. |
| `sample_depths_{480,479}.xlsx` | sample depth registers |
| `ShackletonHall82_d18O.xlsx`, `KeigwinJones90_d18O.xlsx`, `Byrne90_pollen.xlsx` | published age-model tie points |

HPC-side (model, generated by running a script — nothing to download):

| | |
|---|---|
| `{var}.{tag}.0801-0900.tseries.nc` | Years-801-900, 25-variable hyperslab subset of `$PI_CASE_DIR`/`$LGM_CASE_DIR` (campaign storage), via `scripts/nco/subset_tseries.sh`. Flat in `raw/` (no per-case subdirectory) — `tag` is `iPI.01` or `i21ka.03`. Treated as this project's "raw" the same way the proxy `raw/` files are: it's a copy of something that lives outside the repo (campaign storage instead of OneDrive), and nothing edits it after it's written. |
| `{var}_Amon_CESM1.2_LIG127k_146031-182500_cal_adj.nc` | Paleo-calendar-adjusted LIG output, years 401-500, 1200 monthly records per variable. Produced by PaleoCalAdjust v1.1 outside this repo and **staged here 2026-08-10 as the canonical copy**, so `LIG127k_analyses_PALEOCALADJUSTED.ipynb` needs a clone plus this directory and nothing else. Provenance in `DATA_MANIFEST.md` §3 and the notebook's header cell. 19 variables are used (16 isotope tracers + `PRECC`, `PRECL`, `TS`); the rest of the 26 files here are adjusted fields nothing currently reads. |
| `{PHIS,LANDFRAC}.iPI.01.constant.nc` | One record of the PI surface geopotential and land fraction, from the same script and the same campaign tree. Both fields are time-invariant, hence `.constant.nc` rather than a `.tseries.nc` name that would imply a 1200-record window. `PHIS/g` is the model topography `LIG127k_analyses_PALEOCALADJUSTED.ipynb` contours. |
| `topo_21ka_remap_19x25.mod.170428.sm9.nc` | 21 ka topography/land-fraction boundary condition, from `/glade/work/jiangzhu/data/inputdata/cesm120ka_ICEG6/21ka/`. What `LGM_analyses.ipynb` contours — the LGM's ice-sheet topography is not the modern one, so it cannot use the PI `PHIS` above. |

### `external/` — third-party
| | |
|---|---|
| `lr04.mat` | LR04 benthic δ¹⁸O stack (Lisiecki & Raymo 2005). Variable `delob` = `[age (ka), δ¹⁸O, error]`. Read by the MATLAB `icevolcorr` and, since 2026-08-07, by `fig2`. The `LR04stack_d18O.csv` that `fig3` still names no longer exists on this machine — see `DATA_MANIFEST.md`. |
| `OIPC_monthly_data.nc` | OIPC monthly isoscape (~396 MB) |
| `imerg.gn.timeseries.2001-2018.nc` | IMERG precipitation (~3.1 GB). Not read directly by any notebook — `scripts/nco/make_imerg_climo.sh` derives a global climatology and a SW-NA subset from it first, because loading the full field in a Jupyter kernel takes 5.6 GB and kills it. See `DATA_MANIFEST.md`. |
| `obs.etopo5.zsurf.nc` | ETOPO5 topography |
| `GoCregressionFINAL.mat` | Gulf of California δD→%JAS calibration. Provenance still undocumented — see `DATA_MANIFEST.md`. |

### `interim/`
`*_FAMEs_*.mat` from the MATLAB pipeline (laptop-side), and
`{var}.{tag}.0801-0900.tseries.plev.nc` (HPC-side, flat, same `tag` convention as `raw/` above)
— the pressure-level regrid of `raw/` built by `scripts/ncl/pressureRegrid_tseries.ncl`.

### `processed/`
`timeslice_mean_proxy_dDraw.csv` and `timeslice_mean_proxy_dD.csv` — **tracked**, because they
are how the Casper clone receives proxy numbers without any raw proxy data crossing to `/glade`.
Read `data/processed/README.md` for their provenance and the ε caveat: use `dDraw`; the other came
from a cell its author disabled.

## What is currently populated

`raw/` holds a **copy** of the raw, immutable inputs, made 2026-08-05 and verified
byte-identical (md5) against the originals. **Nothing was moved** — every original is still in
place on OneDrive.

```
raw/NH22P/                 dD_nh22p.xls, nh22p_processed_dD_{hand,auto}picked.xlsx
raw/DSDP-480-479/          d480-479_dD.xlsx, d480_d479_processed_dD.xlsx
raw/DSDP-480-479/age_model/ sample_depths_{480,479}.xlsx + the three published tie-point sources
raw/Bacon_runs/DSDP480/    canonical run only — mcmc_new.csv, 165_ages.txt, DSDP480.csv, depths, 165.pdf
raw/Bacon_runs/DSDP479/    mcmc.csv, 113_ages.txt, DSDP479.csv, depths
```

Two deliberate exclusions, both to avoid re-importing ambiguity that has already been resolved:

- **Only the canonical DSDP-480 Bacon run was copied.** The five superseded configurations
  (`_84`, `_162`, `_990`, `_1681`) and the `DSDP480_old*` sibling directories remain on OneDrive
  only. See `DATA_MANIFEST.md` §2 for which run is canonical and why.
- **`external/` is empty**, and the proxy-side `interim/` (`*_FAMEs_*.mat`) is populated only
  after running the MATLAB pipeline. The observational datasets are large (IMERG is 3.1 GB, OIPC
  396 MB) and the MATLAB interim products are pipeline outputs, not inputs. Download or
  regenerate them as needed.

The HPC-side `tseries` files in `raw/` and `interim/` (above) are populated by running
`scripts/nco/subset_tseries.sh` then `scripts/ncl/pressureRegrid_tseries.ncl` — there's no
OneDrive original to copy or verify against, unlike the proxy `raw/` files above; re-running the
scripts is the reproducibility check.

## Two things this layout does not change

**The full iCESM archive stays on `/glade`.** It is far too large to download in full and is
collaborator-staged on NCAR campaign storage; only the small per-variable subset described above
is copied into the repo's own `data/raw/`. Everything else model-side still comes from
the `/glade` block of `config/paths.env`.

**The author's own copies live on OneDrive** (`~/OneDrive/data/{proxies,obs}`) and are not being
moved. `config/paths.env` is what reconciles the two: point `PROXY_DATA_DIR` / `OBS_DATA_DIR` at
`./data/raw` and `./data/external` for a fresh clone, or at OneDrive if that is where your copy
already is. That is exactly what the path config is for — nothing else needs to know.
